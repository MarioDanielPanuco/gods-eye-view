...
// The ocean-field tier logic lives in modules, not in this file: issue #41 asks
// that vite.config.js stop absorbing API implementations, so /api/ocean/field
// is a thin handler over src/server/ocean/*.
import { buildFieldPayload, tryNormalizeBox, DEFAULT_TARGET_CELLS, MIN_TARGET_CELLS, MAX_TARGET_CELLS } from './src/server/ocean/fieldGrid.js';
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        layers: {
          type: 'array',
          items: { type: 'string', enum: ['flights', 'military', 'ais-live-vessels', 'local-firms', 'earthquakes', 'ocean-conditions'] },
          description: 'Layers to query. fires/wildfires → local-firms; ships/vessels → ais-live-vessels; buoys/waves/sea state → ocean-conditions.',
        },
        scope: {
          type: 'object',
          additionalProperties: false,
...
const _oceanFieldRateLimiter = makeRateLimiter({ windowMs: 60_000, max: 12, globalMax: 30 });
...
export function resolveServedAxes(servedLats, servedLons, requested) {
  ...
  if (servedLats.length !== nLat * nLon || servedLons.length !== nLat * nLon) return null;
  if (!servedLats.every(Number.isFinite) || !servedLons.every(Number.isFinite)) return null;
  const tol = OCEAN_NODE_SNAP_TOLERANCE_DEG;
  const lats = [];
  for (let i = 0; i < nLat; i += 1) {
    lats.push(consensusOf(servedLats.slice(i * nLon, (i + 1) * nLon), requested.lats[i], tol));
  }
  const lons = [];
  ...
  for (let i = 0; i < nLat; i += 1) {
    for (let j = 0; j < nLon; j += 1) {
      const node = i * nLon + j;
      if (Math.abs(servedLats[node] - lats[i]) > OCEAN_NODE_SNAP_TOLERANCE_DEG
...
export function georeferenceMarineGrid(axes, grid, nodeCount) {
  ...
  for (const node of dropped) {
    grid.nodes[node] = {
      waveHeightM: [], currentKmh: [], currentDirDeg: [], windMs: [], windDirDeg: [],
    };
  }
  let maxNodeSnapM = 0;
  let maxWindSkewM = 0;
  for (let i = 0; i < axes.lats.length; i += 1) {
    for (let j = 0; j < axes.lons.length; j += 1) {
      ...
      maxNodeSnapM = Math.max(maxNodeSnapM, oceanSeparationM(
        axes.lats[i], axes.lons[j], gridAxes.lats[i], gridAxes.lons[j],
      ));
      if (Number.isFinite(grid.windLats[node]) && Number.isFinite(grid.windLons[node])) {
        maxWindSkewM = Math.max(maxWindSkewM, oceanSeparationM(
          gridAxes.lats[i], gridAxes.lons[j], grid.windLats[node], grid.windLons[node],
        ));
      }
    }
  }
    ...
    middlewares.use('/api/ocean', async (req, res) => {
      ...
      try {
        ...
        if (subPath === '/field') {
          ...
          const requestedCells = rawCells === null ? Number.NaN : Number(rawCells);
          const targetCells = Number.isFinite(requestedCells)
            ? Math.min(MAX_TARGET_CELLS, Math.max(MIN_TARGET_CELLS, Math.trunc(requestedCells)))
          ...
          if (cached && now - cached.cachedAt <= OCEAN_FIELD_CACHE_MS) {
            sendJson(200, { 'Cache-Control': 'public, max-age=300', 'X-Ocean-Field': 'HIT' }, cached.payload);
            return;
          }
          const request = coalesceProxyRequest(_oceanFieldInFlight, key, async () => {
          ...
          try {
            ...
            if (cached && now - cached.cachedAt <= OCEAN_FIELD_STALE_MS) {
              sendJson(200, { 'Cache-Control': 'no-store', 'X-Ocean-Field': 'STALE' }, cached.payload);
              return;
            }
            sendJson(503, { 'Cache-Control': 'no-store' }, { error: 'Ocean current field is temporarily unavailable' });
          }
          return;
        }
