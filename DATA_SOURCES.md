...
| **NOAA NDBC** | Ocean Conditions buoy observations (bulk latest_obs feed + activestations.xml station names) | U.S. Government work, public domain | "NOAA National Data Buoy Center" (courtesy) |
| **Open-Meteo Marine** | Marine forecasts on ocean cards (waves, SST, currents) and the drift-simulation forcing grid (with Open-Meteo forecast winds) | [CC BY 4.0 data licence and adjacent-link attribution requirement](https://open-meteo.com/en/licence) | Linked "Weather data by Open-Meteo.com" |
| **HYCOM ESPC-D-V02** | Primary global surface-current field for the Ocean Currents layer (`FMRC_ESPC-D-V02_uv3z`, `water_u`/`water_v` at depth 0 m) | The dataset declares **no `license` attribute**. Its `distribution_statement` reads "Approved for public release; distribution unlimited" with `classification_level` empty — a U.S. Government public-release statement, which is not itself an explicit grant of reuse terms; none is claimed here | "US Navy Fleet Numerical Meteorology and Oceanography Center (FNMOC)" |
| **NOAA CoastWatch blended currents** | FALLBACK global field when HYCOM is unavailable (`noaacwBLENDEDNRTcurrentsDaily`) | U.S. Government work. The dataset ships its own licence string — free use and redistribution, **not intended for legal use**, no warranty — which the app carries verbatim in the layer's provenance | "Data courtesy of NOAA; Sentinel data courtesy of Copernicus Program; Generated using AVISO+ products" |
| **IOOS HF-radar national network** | Nearshore 0.5–6 km surface-current totals for the Ocean Currents layer (CoastWatch ERDDAP `ucsdHfrW500/W1/W2`, `ucsdHfrE1/E2/E6`, `ucsdHfrH1`, `ucsdHfrP2/P6`) | U.S. Government work, under the same ERDDAP disclaimer licence (free use and redistribution, not for legal use, no warranty), carried verbatim in the layer's provenance | "IOOS HF-radar national network" via NOAA CoastWatch |
...
- **Ocean conditions + drift simulation.** `/api/ocean/obs` fetches NOAA NDBC's bulk latest-observations file server-side on a 10-minute cache (60-minute stale window) and joins station names from a 24-hour-cached `activestations.xml` — the browser only ever sees normalized JSON, and a missing field renders nothing rather than an interpolated guess. NDBC values are **station observations**, not a modeled sea-state field. `/api/ocean/marine` (0.1° cache cells, 15 min TTL) and `/api/ocean/marine-grid` (5×5 × 0.5° forecast grid for the drift simulator, 30 min TTL) supply Open-Meteo Marine and Open-Meteo forecast values, which are **model forecasts** distinct from NDBC observations and are labeled `FC` on cards. `/api/ocean/etopo` supplies NOAA ETOPO1 bathymetry (CoastWatch ERDDAP `etopo180`, U.S. public domain) at 2 arc-min stride over a ±1.5° box for drift beaching — 7-day cache with stale-forever fallback, and the bundled GSHHG mask takes over when it is unreachable, so beaching merely coarsens instead of vanishing. The drift ensemble is labeled `SIMULATED DRIFT ENSEMBLE — NOT A SAR PRODUCT`: coarse forecast currents resolve no nearshore eddies or tidal flow, the MVP has no Stokes drift term, and the output is a probabilistic visualization, not search-and-rescue guidance. Point-in-time captures committed as parser-test fixtures (`src/data/fixtures/ndbc-latest-obs-sample.txt`, `src/data/fixtures/etopo-sample.json`, `src/data/fixtures/hfr-ucsdhfrw2-monterey.csv0`, all U.S. Government works, never served to the app; see `src/data/fixtures/README.md` for each capture's provenance).
- **Ocean Currents field.** `/api/ocean/field` serves the animated surface-current layer over the camera's view rectangle. It is a **two-tier ladder, and every tier that contributes is named on screen with its own age and resolution**. Nearshore it analyses IOOS HF-radar total vectors (CoastWatch ERDDAP `ucsdHfrW500/W1/W2` on the US West Coast, `ucsdHfrE1/E2/E6` on the East Coast and Gulf, `ucsdHfrH1` for Hawai'i, `ucsdHfrP2/P6` for Puerto Rico and the USVI — 0.5 to 6 km, hourly) through QC gates (HDOP ≤ 1.6, |u| and |v| ≤ 2.4 m/s per component, ≥ 40 surviving vectors, and the served hour no more than 12 h old) and a two-pass Barnes objective analysis whose length scale is tied to the observation spacing (L = 2.05·Δ, floored at 2.05 km, putting the half-amplitude wavelength at 4Δ), reporting a holdout cross-validation RMSE over withheld vectors. Those cells are **observed**; the served hour is typically 1–11 h old and its exact age is on the legend. Where radar coverage runs out — which for a coastal view is usually most of it — the remaining water is filled from **HYCOM ESPC-D-V02** (`FMRC_ESPC-D-V02_uv3z`, `water_u`/`water_v` at depth 0 m), a full primitive-equation ocean forecast on a 0.04° lat × 0.08° lon grid at 3-hourly steps spanning about −10 days to +5 days. Unlike the altimetric product it replaced, it carries **wind-driven flow and tides** (eight astronomically forced constituents, with an M2 correction constrained against a tidal atlas; the other seven are unassimilated model error), so it is physically comparable to the HF-radar tier rather than merely adjacent to it. It is a **model forecast, not an observation**, and is labeled `MODELED` with the valid time of the 3-hourly step served — which may be in the FUTURE, since the dataset includes a forecast. When HYCOM is unavailable the tier falls back to NOAA CoastWatch `noaacwBLENDEDNRTcurrentsDaily` (global 0.25°, ~3 days stale), which is **absolute geostrophic velocity only — no Ekman, no tides** and ~300 km effective resolved wavelength; the legend names whichever source actually served and describes its physics. A view served by both is labeled `composite` and states the split ("29% of drawn cells are 1 km HF radar (10 h old); the other 71% is 4.4 km HYCOM global forecast currents (3 h old)"), naming whichever global product actually filled the rest, so an observed 1 km cell and a modeled fill cell are never presented as the same measurement. Coverage is measured over water cells only, using the bundled GSHHG mask, because counting land against a coastal analysis demotes exactly the views HF radar exists to serve; land cells are blanked rather than drawn, both server-side at field resolution and again client-side at 16-pixel screen resolution. A cell with no data is drawn as nothing — never as slack water. Responses are cached 10 minutes on a 0.5°-quantized view box. HF radar is a **coastal** network covering a small fraction of the world ocean, so most of the globe shows the global tier alone — HYCOM where it is reachable, the 0.25° altimetric analysis when it is not. Neither tier resolves Stokes drift, and only the HYCOM tier carries tides: on the altimetric fallback there is no tidal signal at all, which is why the legend states the physics of whichever source served rather than describing the tier once.
